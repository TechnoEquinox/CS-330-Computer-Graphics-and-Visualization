...
private:
	...
	// Render Functions for Apple Lisa Computer (Module Three)
	void DrawGroundPlane();
	void DrawMainBody();
	void DrawBackBase();
	void DrawFeet();
	void DrawFloppyDrive();
	void DrawCRTPanel();

	// Render Functions for ProFile accessory (Module Seven)
	void DrawProFile();

	// Render Functions for Mouse accessory (Module Seven)
	void DrawMouse();
	void DrawMouseCable();

	// Render Function for the Keyboard accessory (Module Seven)
	void DrawKeyboard();
	void DrawKey(float x, float y, float z, float slopeDegrees, glm::vec3 keyScale = glm::vec3(0.3f, 0.1f, 0.3f));
	void DrawKeyboardCable();
